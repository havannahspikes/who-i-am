...
DB_PATH = os.path.join(BASE_DIR, "whoiam.db")
...
def init_db_and_migrate():
    ...
    """)
    # Create uploads if missing (old schema safe)
    db.execute("""
    ...
    """)
    # Create profile table if missing; single-row table to store what appears on life page
    db.execute("""
    ...
    # Ensure profile has one row (id=1)
    ...
    if not r or r["c"] == 0:
        db.execute(
            ...
            (1, "Your Name", "Developer • IT • Cybersecurity", "City, Country", "you@example.com", "https://github.com/yourusername", "https://linkedin.com/in/yourprofile", "https://yourportfolio.example.com", "Degree: BSc Computer Science; Skills: Python, JS, Linux, SQL")
        )
        db.commit()

    # Fill uploaded_at for older rows
...
def alpha():
    ...
    return render_template("alpha.html", hide_register=hide_register, users_exist=users_exist)


# -------------------------
# Life (view-only for employers)
# -------------------------
@app.route("/life")
def life():
    profile = get_profile()
...
# -------------------------
# Admin upload & profile management
# -------------------------
...
# Legacy filename route (keeps compatibility)
...
# -------------------------
# Admin heartbeat (manage users/uploads/profile)
# -------------------------
@app.route("/heartbeat")
def heartbeat():
    if "username" not in session or not session.get("is_admin"):
...
# Deletion routes (admin or uploader, same behavior)
